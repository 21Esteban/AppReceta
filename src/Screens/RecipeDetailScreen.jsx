...
export default function RecipesActionScreen({ route }) {
  ...
  const getReceta = async () =>{
    try {
      setIsLoading(true);
      const { data } = await axios.get(`/recipes/${_id}`);
      setReceta(data.data)
      setIsLoading(false)
    } catch (error) {
      setIsLoading(false)
  ...
  const deleteReceta=async()=>{
    try {
      setIsRemoving(true)
      await axios.delete(`/recipes/${receta._id}`)
    ...
    <ScrollView>
      <View style={styles.imageContainer}>
        <View style={styles.imageBorder}>
          <Image source={{ uri: receta.imgUrl }} style={styles.image} />
        </View>
      </View>

      <View style={{ marginTop: 20 }}>
        <Text style={styles.title}>{receta.name}</Text>
        <Text style={styles.subtitle}>{receta.description}</Text>
      </View>
      <View style={{padding:2}}>
        <TouchableOpacity style={styles.buttonRadius} onPress={()=>navigation.navigate("RecipesActionScreen",receta)}>
          <LinearGradient
        ...
        <TouchableOpacity style={styles.buttonRadius} onPress={()=>deleteReceta()}>
          <LinearGradient
            style={styles.gradient}
            colors={[colors["white"], colors.white]}
          >
